...
#include <windows.h>
//atualiza Tela
void drawScreen(aviao * aviao, tiro * tiro1,tiro * tiro2, tiro * tiro3, meteoro * bloco, meteoro * lanterna, meteoro * quadriculado, meteoro * triangulo, meteoro * circulo, pontuacao * pontuacao, statusTela * statusTela, musicaTocar * musicaTocar)
{

    clear();
    if (statusTela->tela_mostrar == -1 || statusTela->tela_mostrar == 5)
    {
        desenhoion();
        desenhojogar();
        desenhoscore();
    ...
    else if (statusTela->tela_mostrar == 1)
    {
        ...
        setColor(COLOR_MAGENTA,COLOR_BLACK,0);
        desenharAviao(aviao);
        leArqPontos();
        int *v = NULL;
        pontuacao->tam_num = 0;
...
//faz com que o bloco desça somente quando o bloco->y seja igual ao bloco->estado mais 30
    ...
    else if (statusTela->tela_mostrar == 2)
    {

        if (aviao->tecla_clicada == 97)
        {
            musicaTocar->tecla = 97;
        }
        else if (aviao->tecla_clicada == 98)
        {
            musicaTocar->tecla = 98;
        }
        else if (aviao->tecla_clicada == 99)
        {
            musicaTocar->tecla == 99;
        }
        else if (aviao->tecla_clicada == 100)
        {
            musicaTocar->tecla == 100;
        }
        else if (aviao->tecla_clicada == 101)
        {
            musicaTocar->tecla == 101;
        }
        else if (aviao->tecla_clicada == 102)
        {
            musicaTocar->tecla == 102;
        }
        else if (aviao->tecla_clicada == 103)
        {
            musicaTocar->tecla == 103;
        }
        else if (aviao->tecla_clicada == 104)
        {
            musicaTocar->tecla == 103;
        }
        Tocarmusica(aviao);
    ...
    else if (statusTela->tela_mostrar==3)
    {
        ajuda();
    }

    else if (statusTela->tela_mostrar == 6)
    {
        setColor(COLOR_RED,COLOR_BLACK,0);
        if (pontuacao->ponto >0){
            escreverNumeros(pontuacao->ponto);
        }

        int *v = NULL;
        int tam = 0;
        v = leArquivosPontuacoes("pontuacoes.txt", &tam);
        ordenar(v, tam);
        imprimeNumeros(v, tam);
        reconfig_aviao_meteoros(aviao,bloco, lanterna, quadriculado, triangulo, circulo);
        pontuacao->ponto = 0;
...
void handleInputs(aviao * aviao, statusTela * statusTela)
{
    ...
    case '1':
        ...
        break;
    case '2':
        statusTela->tela_mostrar = 2;
        aviao->tecla_clicada = -1;
        break;
    case '3':
        statusTela->tela_mostrar = 3;
        aviao->tecla_clicada = -1;
        break;
    case '4':
        curs_set(1);

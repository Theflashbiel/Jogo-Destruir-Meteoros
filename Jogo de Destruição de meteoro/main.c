...
int main(int argc, char *argv[])
{
    ...
    musicaTocar musicaTocar;


    // inicializa a tela pelo Curses e o estado inicial da aplicação

    initScreen();
    initGame(&aviao, &tiro1, &tiro2, &tiro3, &bloco, &lanterna, &quadriculado, &triangulo, &circulo, &pontuacao, &statusTela, &musicaTocar);
	...
	while(1)
    {
        ...
        // Atualiza a tela
        drawScreen(&aviao, &tiro1, &tiro2, &tiro3, &bloco, &lanterna, &quadriculado, &triangulo, &circulo, &pontuacao, &statusTela, &musicaTocar);
